...
class Player:
    ...
    def ev_calc(self, deck):
        expected_val = 0
        current_pts = self.score()
        for card in deck:
            if isinstance(card, int):
                if card not in self.hand:
                    expected_val += card
                if card in self.hand:
                    expected_val -= current_pts
        return expected_val
...
def handle_action(card, deck, player, opponent):
    ...
    if card == 'flip 3':
        ...
        # nullify the randomization under these conditions
        if player == variant:
            if player.active and opponent.active:
                if opponent.ev_calc(deck) < 0:
                    target = opponent
                elif player.ev_calc(deck) > 0:
                    target = player
                else:
                    target = opponent
...
with open('results_all.csv', 'w') as myfile:
    ...
    for i in range(1000):
        ...
        while baseline.total_score < 200 and variant.total_score < 200:
            ...
            while baseline.active or variant.active:
                ...
                if variant.active:
                    # variant's turn
                    if variant.ev_calc(deck) < 0 and '2nd chance' not in variant.hand:
